...
def _pick(rows: list[dict[str, Any]], symbol: str, system: str = "", direction: str = "", result: str = "") -> dict[str, Any] | None:
    ...
    result = html.unescape(result).strip().upper()
    for row in rows:
        ...
        if direction and cards.direction(row) != direction:
            continue
        if result and cards.outcome(row) != result:
            continue
        return row
    return None
