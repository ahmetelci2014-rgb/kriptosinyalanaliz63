# pump_radar.py
# Erken Pump/Dump Radar
# OKX USDT Futures tarar. Emir açmaz; Telegram uyarısı ve TP/SL takibi yapar.
...
BOT_NAME = "Erken Pump/Dump Radar"
...
PUMP_MIN_CLOSE_POWER_5M = 56
DUMP_MAX_CLOSE_POWER_1M = 42
DUMP_MAX_CLOSE_POWER_5M = 44
...
def load_state():
    try:
        ...
        # Eski state dosyalarıyla uyum.
        data.setdefault("open_signals", {})
        data.setdefault("open_pump_signals", {})
        if data.get("open_pump_signals") and not data.get("open_signals"):
            data["open_signals"] = data.get("open_pump_signals", {})
...
def save_state(state):
    try:
        # Eski workflow/dosya alışkanlıkları bozulmasın diye iki anahtar da tutulur.
        state["open_pump_signals"] = state.get("open_signals", {})
...
def recent_resistance(df5):
    try:
        if len(df5) < BREAKOUT_LOOKBACK_5M + 5:
            return None
        return float(df5["high"].iloc[-BREAKOUT_LOOKBACK_5M - 2:-2].max())
    except Exception:
        return None


def recent_support(df5):
    try:
        if len(df5) < BREAKOUT_LOOKBACK_5M + 5:
            return None
        return float(df5["low"].iloc[-BREAKOUT_LOOKBACK_5M - 2:-2].min())
    except Exception:
        return None
...
def missing_reasons(conditions):
    return [item["label"] for item in conditions if not item["ok"]]


def score_from_conditions(conditions, bonus=0):
    ok_count = sum(1 for item in conditions if item["ok"])
...
def has_open_same_symbol(state, symbol):
    for signal in state.get("open_signals", {}).values():
...
def build_signal_message(signal):
    ...
    return (
        ...
        f"• 1M Kapanış Gücü: %{round(signal['close_power1'], 1)}\n"
        f"• 5M Kapanış Gücü: %{round(signal['close_power5'], 1)}\n"
        f"• Kırılım Seviyesi: {format_price(signal['break_level'])}\n\n"
        f"📌 İşlem Kuralı:\n"
        f"• Erken pump/dump sinyalidir, risk yüksektir.\n"
...
def build_long_signal(symbol, current_price, df1, df5, df15):
    try:
        df1 = df1.copy()
        df5 = df5.copy()
        ...
        cp1 = close_power_percent(c1)
        cp5 = close_power_percent(c5)
        lw1 = lower_wick_percent(c1)

        resistance = recent_resistance(df5)
        if resistance is None:
            return None, None

        breakout = (
            float(c5["close"]) >= resistance * (1 - BREAKOUT_TOLERANCE_PERCENT / 100)
            or float(c5["high"]) >= resistance * (1 - BREAKOUT_TOLERANCE_PERCENT / 100)
            or entry >= resistance * (1 - BREAKOUT_TOLERANCE_PERCENT / 100)
        )

        raw_sl = min(float(c1["low"]), float(c5["low"]), resistance * 0.995)
        ...
        conditions = [
            condition("1M yeşil atak yetersiz", move1 >= MIN_1M_MOVE),
            condition("5M pump hareketi yetersiz", move5 >= MIN_5M_MOVE),
            condition("15M yön desteği yetersiz", move15 >= MIN_15M_MOVE),
            condition("1M hacim düşük", vol1 >= MIN_1M_VOLUME_RATIO),
            condition("5M hacim düşük", vol5 >= MIN_5M_VOLUME_RATIO),
            condition("direnç kırılımı yok", breakout),
            condition("1M kapanış gücü zayıf", cp1 >= PUMP_MIN_CLOSE_POWER_1M),
            condition("5M kapanış gücü zayıf", cp5 >= PUMP_MIN_CLOSE_POWER_5M),
            condition("5M RSI uygun değil", PUMP_RSI_5M_MIN <= rsi5 <= PUMP_RSI_5M_MAX),
            condition("risk uygun değil", MIN_RISK_PERCENT <= risk_percent <= MAX_RISK_PERCENT),
        ]
        ...
        hard_ok = (
            ...
            and (move5 >= MIN_5M_MOVE or breakout)
            and (vol1 >= MIN_1M_VOLUME_RATIO or vol5 >= MIN_5M_VOLUME_RATIO)
            and cp1 >= 52
            and rsi5 <= PUMP_RSI_5M_MAX
        )
        ...
        print(symbol, "pump long analiz hatası:", e)
        return None, None


def build_short_signal(symbol, current_price, df1, df5, df15):
    try:
        df1 = df1.copy()
        df5 = df5.copy()
        ...
        cp1 = close_power_percent(c1)
        cp5 = close_power_percent(c5)
        uw1 = upper_wick_percent(c1)

        support = recent_support(df5)
        if support is None:
            return None, None

        breakdown = (
            float(c5["close"]) <= support * (1 + BREAKOUT_TOLERANCE_PERCENT / 100)
            or float(c5["low"]) <= support * (1 + BREAKOUT_TOLERANCE_PERCENT / 100)
            or entry <= support * (1 + BREAKOUT_TOLERANCE_PERCENT / 100)
        )

        raw_sl = max(float(c1["high"]), float(c5["high"]), support * 1.005)
        sl = raw_sl * (1 + SL_BUFFER_PERCENT / 100)
        risk = sl - entry
        if risk <= 0:
            return None, None
        ...
        conditions = [
            condition("1M kırmızı atak yetersiz", move1 <= -MIN_1M_MOVE),
            condition("5M dump hareketi yetersiz", move5 <= -MIN_5M_MOVE),
            condition("15M yön desteği yetersiz", move15 <= -MIN_15M_MOVE),
            condition("1M hacim düşük", vol1 >= MIN_1M_VOLUME_RATIO),
            condition("5M hacim düşük", vol5 >= MIN_5M_VOLUME_RATIO),
            condition("destek kırılımı yok", breakdown),
            condition("1M kapanış gücü zayıf", cp1 <= DUMP_MAX_CLOSE_POWER_1M),
            condition("5M kapanış gücü zayıf", cp5 <= DUMP_MAX_CLOSE_POWER_5M),
            condition("5M RSI uygun değil", DUMP_RSI_5M_MIN <= rsi5 <= DUMP_RSI_5M_MAX),
            condition("risk uygun değil", MIN_RISK_PERCENT <= risk_percent <= MAX_RISK_PERCENT),
        ]
        ...
        hard_ok = (
            MIN_RISK_PERCENT <= risk_percent <= MAX_RISK_PERCENT
            and (move5 <= -MIN_5M_MOVE or breakdown)
            and (vol1 >= MIN_1M_VOLUME_RATIO or vol5 >= MIN_5M_VOLUME_RATIO)
            and cp1 <= 48
            and rsi5 >= DUMP_RSI_5M_MIN
        )
        ...
        print(symbol, "dump short analiz hatası:", e)
        return None, None
...
def analyze_symbol(exchange, symbol):
    ...
    df5 = fetch_df(exchange, symbol, "5m", limit=120, min_len=70)
    df15 = fetch_df(exchange, symbol, "15m", limit=90, min_len=50)

    if df1 is None or df5 is None or df15 is None:
        return [], None, None

    long_signal, long_debug = build_long_signal(symbol, current_price, df1, df5, df15)
    short_signal, short_debug = build_short_signal(symbol, current_price, df1, df5, df15)
...
def build_no_signal_report(scanned_count, new_signal_count, pump_counter, dump_counter, top_candidates):
    lines = [
        f"🚨 {BOT_NAME} RAPORU\n",
        f"Zaman: {tr_now_text()}",
        f"Taranan coin: {scanned_count}",
        f"Yeni pump/dump sinyal: {new_signal_count}\n",
        f"PUMP tarafında en çok elenen:",
    ...
    if top_candidates:
        for item in top_candidates[:TOP_NEAR_CANDIDATES]:
    ...
    lines.append(
        ...
        "Pump/dump radarın hangi filtrelerden dolayı sinyal üretmediğini gösterir."
    )
...
def main():
    ...
    open_count = len(state.get("open_signals", {}))
    available_slots = max(0, MAX_OPEN_SIGNALS - open_count)

    print("Açık pump/dump:", open_count)
    print("Boş slot:", available_slots)

    all_signals = []
    pump_reasons = Counter()
    dump_reasons = Counter()
    top_candidates = []
    scanned = 0
    ...
    print("Bulunan pump/dump sinyal:", len(all_signals))
    print("Gönderilecek pump/dump sinyal:", len(selected))

    if selected:
        send_telegram(
            f"🚨 {BOT_NAME} çalıştı.\n"
            f"Taranan coin: {scanned}\n"
            f"Bulunan pump/dump sinyal: {len(all_signals)}\n"
            f"Açık sinyal: {open_count}/{MAX_OPEN_SIGNALS}\n"
            f"Gönderilecek sinyal: {len(selected)}"
        )

    if all_signals and available_slots <= 0:
        send_telegram(
            f"⚠️ {BOT_NAME} güçlü aday buldu ama açık sinyal sınırı dolu.\n"
            f"Açık sinyal: {open_count}/{MAX_OPEN_SIGNALS}\n"
