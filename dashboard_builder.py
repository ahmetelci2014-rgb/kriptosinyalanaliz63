...
VERSION = "KRIPTO_KONTROL_PANELI_V1_6_2026_08_14"
...
def render_dashboard(
    ...
    *,
    ...
    function renderAll() {
      ...
      applyViewerPermissions(); initHeader(); renderKpis(); renderSystems(); renderRisk(); renderPerformance(); renderDirection(); renderComparison(); renderSources(); renderOpen(); renderResults(); renderHealth();
    }
...
"""
        ...
        bootstrap_script = (
            "    applyViewerPermissions(); initHeader(); renderKpis(); renderSystems(); "
...
<head>
  ...
  <style{nonce_attr}>
    ...
    *{{box-sizing:border-box}} html{{scroll-behavior:smooth}} body{{margin:0;background:radial-gradient(circle at 85% 0,#10342f 0,transparent 28%),radial-gradient(circle at 0 35%,#0b2533 0,transparent 25%),var(--bg);color:var(--text);font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh}}
    ...
    body:not([data-viewer-role]) [data-admin-only]{{display:none!important}}
...
<body>
  ...
  <main id="top" class="shell">
    <section class="hero"><div class="hero-copy"><div class="eyebrow">{eyebrow}</div><h1>Gerçek sinyaller.<br><span>Tek kontrol ekranı.</span></h1><p id="heroLead" class="lead">Premium, Scalp, Pump/Dump ve Yeni Liste işlemleri; TP/SL sonuçları ve System Control sağlığı aynı panelde. Bu ekran veri gösterir, işlem açmaz.</p><div class="safety"><span class="badge">{safety_badge}</span><span class="badge">✓ Telegram göndermez</span><span class="badge">✓ Stratejiye dokunmaz</span></div></div><div class="health-hero"><div id="healthRing" class="health-ring"><div><strong id="healthScore">—</strong><span>SİSTEM SAĞLIĞI</span></div></div><div class="health-meta"><strong id="overallHealth">—</strong><p id="healthBreakdown">—</p><p id="healthAge">—</p></div></div></section>
    <div id="qualityWarning" class="quality-warning"></div>
    <nav class="quick-nav" aria-label="Panel bölümleri"><a data-admin-only href="#systems">Sistemler</a><a data-admin-only href="#risk">Açık Risk</a><a href="#performance">Performans</a><a href="#direction-performance">Yön / Gün</a><a data-admin-only href="#period-comparison">Dönem</a><a data-admin-only href="#sources">Veri</a>{market_nav_link}<a href="#open">Açık İşlemler</a><a href="#results">Geçmiş</a><a data-admin-only href="#health">Sağlık</a></nav>
    <section id="kpis" class="kpis"></section>

    <section class="section" id="systems" data-admin-only><div class="section-head"><div><h2>Canlı Sistemler</h2><p>Gerçek işlem sinyali üreten veya gerçek sinyal takibi yapan katmanlar</p></div></div><div id="systemGrid" class="system-grid"></div></section>

    <section class="section" id="risk" data-admin-only><div class="section-head"><div><h2>Açık Risk Özeti</h2><p>Açık kayıtların yön, stop mesafesi ve hedef/risk görünümü; bilgi amaçlıdır, emir üretmez</p></div></div><div class="panel risk-panel"><div id="riskGrid" class="risk-grid"></div><div id="riskSystems" class="risk-systems"></div><div id="riskNote" class="risk-note"></div></div></section>
    ...
    <section class="section" id="period-comparison" data-admin-only><div class="section-head"><div><h2>Dönem Karşılaştırması</h2><p>Son dönemi önceki eşit dönemle sistem sistem karşılaştır; karar verirken örnek sayısını Net R ile birlikte değerlendir</p></div><select id="comparisonWindow" class="select-filter" aria-label="Karşılaştırma dönemi"><option value="7D">7 gün / önceki 7 gün</option><option value="30D" selected>30 gün / önceki 30 gün</option></select></div><div id="periodGrid" class="period-grid"></div></section>

    <section class="section" id="sources" data-admin-only><div class="section-head"><div><h2>Veri Güncelliği</h2><p>Her veri kaynağı ayrı kontrol edilir; eski veya zamanı belirsiz kritik kayıtlar uyarı üretir</p></div></div><div id="sourceGrid" class="source-grid"></div></section>
...
{market_section}
  ...
  <script{nonce_attr}>
    ...
    function applyViewerPermissions() {{
      const role=String(DASHBOARD_DATA.viewer?.role||(LIVE_ENDPOINT?"MEMBER":"ADMIN")).toUpperCase(),isAdmin=role==="ADMIN";
      document.body.dataset.viewerRole=role;
      document.querySelectorAll("[data-admin-only]").forEach(element=>{{element.hidden=!isAdmin;}});
      if(!isAdmin)document.getElementById("heroLead").textContent="Aktif sinyaller, coin grafikleri ve gerçekleşmiş TP/SL sonuçları güvenli üye görünümünde. Bu ekran veri gösterir, işlem açmaz.";
    }}
