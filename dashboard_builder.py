...
VERSION = "KRIPTO_KONTROL_PANELI_V1_2026_08_14"
...
<head>
  ...
  <style{nonce_attr}>
    ...
    *{{box-sizing:border-box}} html{{scroll-behavior:smooth}} body{{margin:0;background:radial-gradient(circle at 85% 0,#10342f 0,transparent 28%),radial-gradient(circle at 0 35%,#0b2533 0,transparent 25%),var(--bg);color:var(--text);font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh}}
    ...
    @media(max-width:1050px){{.hero{{grid-template-columns:1fr}}.kpis{{grid-template-columns:repeat(3,1fr)}}.system-grid{{grid-template-columns:repeat(2,1fr)}}.health-grid{{grid-template-columns:repeat(2,1fr)}}.result-layout{{grid-template-columns:1fr}}}}
    @media(max-width:680px){{.shell{{width:min(100% - 20px,1480px)}}.topbar{{height:auto;min-height:70px;padding:10px 0}}.brand small,.top-actions .badge:nth-child(2){{display:none}}main{{padding-top:24px}}.hero-copy{{padding:24px}}.health-hero{{justify-content:flex-start}}.kpis{{grid-template-columns:repeat(2,1fr)}}.kpi:last-child{{grid-column:1/-1}}.system-grid,.health-grid{{grid-template-columns:1fr}}.section-head{{align-items:flex-start;flex-direction:column}}.toolbar{{align-items:stretch;flex-direction:column}}.search{{width:100%}}footer{{flex-direction:column}}}}
  </style>
</head>
<body>
