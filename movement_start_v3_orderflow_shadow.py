...
- TRIGGER adayları için kota rezerve ederek tarama sırasının güçlü adayları boğmasını engeller.
...
VERSION = "MOVEMENT_START_V3_OKX_ORDERFLOW_PRIORITY_SHADOW_2026_08_20"
...
# Tarama sırası nedeniyle ilk PREP/ARMED adayları tüm kotayı tüketmesin.
# PREP en fazla ilk 2 toplam sorguyu, ARMED en fazla ilk 5 toplam sorguyu
# kullanabilir; kalan en az 5 slot TRIGGER adayları için korunur.
RESERVED_TRIGGER_SLOTS = 5
RESERVED_ARMED_SLOTS = 3
...
_DIRTY = False
_QUERY_COUNT = 0
_QUERY_BY_STAGE: Counter = Counter()
_SKIPPED_BY_PRIORITY: Counter = Counter()
...
def begin(path: str = STATE_FILE) -> None:
    global _STATE, _STATE_PATH, _DIRTY, _QUERY_COUNT, _QUERY_BY_STAGE, _SKIPPED_BY_PRIORITY
    _STATE_PATH = path
    _STATE = _load(path)
    _DIRTY = False
    _QUERY_COUNT = 0
    _QUERY_BY_STAGE = Counter()
    _SKIPPED_BY_PRIORITY = Counter()
...
def fetch_order_flow(symbol: str, now_ts: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    recent_trade_imbalance = (
        (recent_buy - recent_sell) / recent_total if recent_total > 0 else 0.0
    )
...
def score_order_flow(
    ...
    conditions = {
        ...
        "strong_flow": (book >= 0.18 and recent >= 0.18)
        or (trades >= 0.22 and recent >= 0.22),
    }
    weights = {
        "book_support": 18,
        "top_support": 9,
...
def _priority_slot_available(base_result: Optional[Dict[str, Any]]) -> bool:
    """Reserve REST budget for higher-value stages without exceeding 10 queries."""
    global _SKIPPED_BY_PRIORITY

    if not should_query(base_result):
        return False

    stage = str((base_result or {}).get("stage") or "").upper()
    if _QUERY_COUNT >= MAX_ORDERFLOW_QUERIES_PER_RUN:
        _SKIPPED_BY_PRIORITY[f"{stage}:TOTAL_CAP"] += 1
        return False

    prep_ceiling = (
        MAX_ORDERFLOW_QUERIES_PER_RUN
        - RESERVED_TRIGGER_SLOTS
        - RESERVED_ARMED_SLOTS
    )
    if stage == "PREP" and _QUERY_COUNT >= prep_ceiling:
        _SKIPPED_BY_PRIORITY["PREP:RESERVED_FOR_ARMED_TRIGGER"] += 1
        return False

    armed_ceiling = MAX_ORDERFLOW_QUERIES_PER_RUN - RESERVED_TRIGGER_SLOTS
    if stage == "ARMED" and _QUERY_COUNT >= armed_ceiling:
        _SKIPPED_BY_PRIORITY["ARMED:RESERVED_FOR_TRIGGER"] += 1
        return False
...
def _update_open(symbol: str, df5m: Any, price: float, now: int) -> None:
    ...
    for record in state.get("open", {}).values():
        ...
        record["max_favorable_r"] = round(
            max(_sf(record.get("max_favorable_r")), favorable / risk), 4
        )
        record["max_adverse_r"] = round(
            max(_sf(record.get("max_adverse_r")), adverse / risk), 4
        )
...
def observe(
    ...
    *,
    ...
    global _DIRTY, _QUERY_COUNT, _QUERY_BY_STAGE
    ...
    if not _priority_slot_available(base_result):
        return None

    stage = str((base_result or {}).get("stage") or "").upper()
    _QUERY_COUNT += 1
    _QUERY_BY_STAGE[stage] += 1
    ...
    if isinstance(active, dict):
        ...
        if confirmed and not active.get("first_confirmed_at"):
            ...
            active["first_confirmed_minutes"] = round(
                (now - int(active.get("started_at") or now)) / 60.0, 2
            )
...
def _summarize() -> Dict[str, Any]:
    ...
    return {
        ...
        "queries_by_stage_this_run": dict(_QUERY_BY_STAGE),
        "skipped_by_priority_this_run": dict(_SKIPPED_BY_PRIORITY),
        ...
        "decisive_r2_rate": (
            round(status.get("R2_FIRST", 0) / decisive * 100.0, 2)
            if decisive
            else None
        ),
...
def finish(now_ts: Optional[int] = None) -> Dict[str, Any]:
    ...
    state["records"] = [
        r for r in state.get("records", []) if int(r.get("started_at") or 0) >= cutoff
    ][-MAX_RECORDS:]
    state["snapshots"] = [
        s for s in state.get("snapshots", []) if int(s.get("at") or 0) >= cutoff
    ][-MAX_SNAPSHOTS:]
    state["last_started"] = {
        k: v
        for k, v in state.get("last_started", {}).items()
